<?php
  ...
  class StoreConfigBarcodeController
  {
      public function viewStoreConfigBarcode($request , $response)
      {
        $db = pdo();
        $selectStoreConfig = $db->select()->from('store_config_barcode');
     ...
     if($data != null){
        return $response->withHeader('Content-Type' , 'application/json')
                        ->withJson(["Store Config Barcode" => $data]);
      } else {
      ...
      public function updateStoreConfigBarcode($request , $response)
      {
